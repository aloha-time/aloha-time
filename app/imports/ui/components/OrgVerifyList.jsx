...
import OrgVerifyItem from './OrgVerifyItem';
...
function OrgVerifyList({ ready, organizations }) {
  ...
  const handleSearchChange = React.useCallback((e, data) => {
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      dispatch({
        type: 'FINISH_SEARCH',
        results: _.filter(organizations, isMatch),
      });
    }, 300);
  }, []);
  React.useEffect(() => () => {
  ...
  return ((ready) ? (
    <Grid container divided='vertically'>
      ...
      <Grid.Row>
        <Grid.Column>
          <Item.Group divided>
            {results.map((org) => <OrgVerifyItem key={organizations._id} organization={org}/>)}
            {organizations.map((org) => <OrgVerifyItem key={organizations._id} organization={org}/>)}
          </Item.Group>
        </Grid.Column>
      </Grid.Row>
    </Grid>
...
export default withTracker(() => {
  ...
  const organizations = OrganizationProfiles.find({}, { sort: { organizationName: 1 } }).fetch();
  return {
    organizations,
    ready,
  };
